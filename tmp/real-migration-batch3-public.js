...
const REPORT_PATH = 'tmp/real-migration-batch3-public-report.json';
const MAX_ASSETS = 50;
...
(async () => {
  ...
  const report = {
    ...
    mode: 'REAL_PUBLIC_MIGRATION_BATCH3',
    constraints: {
  ...
  for (const target of selectedTargets) {
    ...
    try {
      ...
      handleUnexpectedError(report, `Upload failed ${target.exhibitionId}/${target.itemId}/${target.kind}: ${error.message || String(error)}`);
    }
    ...
    if (!uploadedUrl || !isPublicBlobUrl(uploadedUrl) || !publicCheck.ok || !publicCheck.isImage) {
      handleUnexpectedError(report, `Validation failed ${target.exhibitionId}/${target.itemId}/${target.kind}: status=${publicCheck.status} contentType=${publicCheck.contentType}`);
    }
