...
const REPORT_PATH = 'tmp/real-migration-batch2-public-report.json';
const MAX_ASSETS = 20;
...
(async () => {
  ...
  const report = {
    ...
    mode: 'REAL_PUBLIC_MIGRATION_BATCH2',
    constraints: {
  ...
  if (selectedTargets.length === 0) {
    throw new Error('No eligible candidates found for batch migration.');
  }
  ...
  for (const target of selectedTargets) {
    ...
    if (!item) {
      ...
      report.errors.push(`Target item missing in working state: ${target.exhibitionId}/${target.field}/${target.itemId}`);
      continue;
    }
    ...
    if (!isDataUrl(dataValue)) {
      report.counters.failed += 1;
      report.errors.push(`Missing data URL at migration time: ${target.exhibitionId}/${target.field}/${target.itemId}/${target.kind}`);
      continue;
    }
    ...
    if (!parsed || !parsed.buffer || parsed.buffer.length === 0) {
      report.counters.failed += 1;
      report.errors.push(`Failed to parse data URL: ${target.exhibitionId}/${target.field}/${target.itemId}/${target.kind}`);
      continue;
    }

    const digest = createHash('sha1').update(parsed.buffer).digest('hex').slice(0, 16);
    const ext = extFromMime(parsed.mimeType);
    const path = `exhibition-images/${target.exhibitionId}/${target.kind}/public-batch2-${target.itemId}-${digest}.${ext}`;
    ...
    try {
      ...
      report.counters.failed += 1;
      report.errors.push(`Upload failed ${target.exhibitionId}/${target.itemId}/${target.kind}: ${error.message || String(error)}`);
      continue;
    }
    ...
    if (!uploadedUrl || !isPublicBlobUrl(uploadedUrl) || !publicCheck.ok || !publicCheck.isImage) {
      report.counters.failed += 1;
      report.errors.push(`Validation failed ${target.exhibitionId}/${target.itemId}/${target.kind}: status=${publicCheck.status} contentType=${publicCheck.contentType}`);
      continue;
    }
